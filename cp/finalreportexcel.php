...
if(isset($_SESSION['user_type'])&&isset($_SESSION['username'])){
    if($_SESSION['user_type']==0&&isset($_GET['pid'])){
        $sql =$dbc->selectAllFinalReport();
        reportGeneration($_GET['o_name'],$_GET['pid'],$sql);
    }else if(isset($_GET['oid'])){
        $dbc->generateEducationalIndividualReport($_GET['oid']);
        $sql =$dbc->selectIndividualEduReport();
        reportGeneration($_GET['o_name'],0,$sql);
    }
    else if(isset($_GET['lid'])){
        $dbc->generateIndividualLocalReport($_GET['lid']);
        $sql =$dbc->selectIndividualLocalReport();
        reportGeneration($_GET['o_name'],0,$sql);
    }
    else{
...
function reportGeneration($officeName,$projectId,$sql){
    ...
    if(isset($_GET['o_name'])){
        $objPHPExcel->setActiveSheetIndex()->setCellValue('D1',"प्रगति प्रतिवेदन (".$officeName.")");
    }else{
    ...
    $objPHPExcel->setActiveSheetIndex()->setCellValue('D3',"कार्यक्रम : ".$projectId==0?'':$projectId." (विद्यालयक्षेत्र विकास कार्यक्रम)");
    $objPHPExcel->setActiveSheetIndex()
        ->mergeCells('A4:A5');
    ...
    ob_end_clean();
    if(isset($_GET['o_name'])){
        header('Content-Disposition: attachment;filename="pis-report-final-for-'.$officeName.'.xls"');
    }else{
